/**
 * SimSuite DOM Inspector - Uses /mnt/c/ paths for file I/O
 */
...
const EXE_WIN = '/mnt/c/Users/likwi/OneDrive/Desktop/PROJS/SimSort/src-tauri/target/debug/simsuite.exe';
...
function runPS(psContent) {
  const ps1 = PROJ + '/cdp_tmp.ps1';
  const resultFile = PROJ + '/cdp_result.txt';

  // Write PS1 with forward-slash paths
  let fullPS = psContent
    .replace('__EXE__', EXE_WIN)
    .replace('__PORT__', String(PORT))
    .replace('__RESULT__', resultFile);

  fs.writeFileSync(ps1, fullPS, 'utf8');

  // Execute PS1 directly via PowerShell
  try {
    execSync('/mnt/c/Windows/System32/WindowsPowerShell/v1.0/powershell.exe -NoProfile -ExecutionPolicy Bypass -File "' + ps1 + '"', {
      windowsHide: true,
      timeout: 30000
    });
  } catch {}
  ...
  let result = '';
  try { result = fs.readFileSync(resultFile, 'utf8').trim(); } catch {}
