/**
 * SimSuite DOM Inspector — reads CDP info from file, runs inspection
 * Usage: node test-dom2.cjs <cdpInfoFile>
 */
...
if (!cdpFile) { console.error('Usage: node test-dom2.cjs <cdpInfoFile>'); process.exit(1); }

const cdpInfo = JSON.parse(fs.readFileSync(cdpFile, 'utf8'));
const wsUrl = cdpInfo.wsUrl;
const port = cdpInfo.port;
...
async function main() {
  ...
  console.log('   WS URL:', wsFullUrl.substring(0, 60), '...\n');
