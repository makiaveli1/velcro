#!/usr/bin/env python3
"""Apply right rail + utility drawer replacement to ConceptCanvas.jsx"""

with open('/home/likwid/.openclaw/workspace/ventures/velcro/ventures/website-studio/CRM/ui/src/pages/ConceptCanvas.jsx', 'r') as f:
    content = f.read()

rail_start = content.find('      {/* ── Right Rail ─────────────────────────────────────────────── */}')
cp_close = content.find('    </div>\n  );\n}', rail_start)

# Verify we found the right markers
assert content[rail_start:rail_start+10] == '      {/*', f"rail_start wrong: {content[rail_start:rail_start+30]!r}"
assert content[cp_close:cp_close+10] == '    </div>', f"cp_close wrong: {content[cp_close:cp_close+30]!r}"
...
print(f"New rail: {len(new_right_rail)} chars")

# Verify new_right_rail ends correctly (should NOT have canvas-page </div>)
last100 = new_right_rail[-100:]
print("New rail last 100 chars:", repr(last100))

# Strip the canvas-page closing </div> if it's accidentally included
# The canvas-page close is "    </div>" preceded by newlines
canvas_close_idx = new_right_rail.rfind('\n    </div>')
if canvas_close_idx != -1:
    print(f"Found canvas-page </div> at idx {canvas_close_idx}, stripping...")
    new_right_rail = new_right_rail[:canvas_close_idx] + '\n    '
    print("After strip, ends with:", repr(new_right_rail[-30:]))

new_content = content[:rail_start] + new_right_rail + content[cp_close:]
print(f"\nTotal new file: {len(new_content)} chars")
print(f"Old file: {len(content)} chars")

with open('/home/likwid/.openclaw/workspace/ventures/velcro/ventures/website-studio/CRM/ui/src/pages/ConceptCanvas.jsx', 'w') as f:
    f.write(new_content)
